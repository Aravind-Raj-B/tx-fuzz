...
type Config struct {
	...
	faucet     *slhdsa.PrivateKey   // private key of the faucet account
	keys       []*slhdsa.PrivateKey // private keys of accounts
...
func NewDefaultConfig(rpcAddr string, N uint64, accessList bool, rng *rand.Rand) (*Config, error) {
	...
	// Setup Keys
	var keys []*slhdsa.PrivateKey
	for i := 0; i < len(staticKeys); i++ {
		keys = append(keys, crypto.ToSLHDSAUnsafe(common.FromHex(staticKeys[i])))
	}

	return &Config{
		backend:    backend,
		N:          N,
		faucet:     crypto.ToSLHDSAUnsafe(common.FromHex(txfuzz.SK)),
		keys:       keys,
		corpus:     [][]byte{},
...
func NewPartialConfig(backend *rpc.Client, faucet *slhdsa.PrivateKey, keys []*slhdsa.PrivateKey) *Config {
	return &Config{
		backend: backend,
		faucet:  faucet,
		keys:    keys,
...
func NewConfigFromContext(c *cli.Context) (*Config, error) {
	...
	if sk := c.String(flags.SkFlag.Name); sk != "" {
		faucet, err = crypto.ToSLHDSA(common.FromHex(sk))
		if err != nil {
			return nil, err
		}
	}
