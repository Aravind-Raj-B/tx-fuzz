...
func SendTx(sk *slhdsa.PrivateKey, backend *ethclient.Client, to common.Address, value *big.Int) (*types.Transaction, error) {
...
func sendTxWithNonce(sk *slhdsa.PrivateKey, backend *ethclient.Client, to common.Address, value *big.Int, nonce uint64) (*types.Transaction, error) {
...
func sendRecurringTx(sk *slhdsa.PrivateKey, backend *ethclient.Client, to common.Address, value *big.Int, numTxs uint64) (*types.Transaction, error) {
	sender := crypto.PubkeyToAddress(sk.PublicKey)
	nonce, err := backend.NonceAt(context.Background(), sender, nil)
	if err != nil {
		return nil, err
...
func tryUnstuck(config *Config, sk *slhdsa.PrivateKey) error {
	var (
