...
func test7702NormalTxs() {
	...
	sendNormalTxs := func(keys []*slhdsa.PrivateKey) {
...
func test7702BlobTxs() {
	...
	nonceOffset = 0
	keys := spammer.CreateAddressesRaw(NumKeys)
	sendBlobTxs := func(keys []*slhdsa.PrivateKey) {
		cl, _ := helper.GetRealBackend()
		backend := ethclient.NewClient(cl)
		for _, key := range keys {
			sender := crypto.PubkeyToAddress(key.PublicKey)
...
func test7702Scenario(keys []*slhdsa.PrivateKey, value *big.Int, sendTxs func(keys []*slhdsa.PrivateKey)) {
...
func sendAuths(keys []*slhdsa.PrivateKey, callerAddr, calleeAddr common.Address, senderNonce uint64) *types.Transaction {
...
func verify(client *rpc.Client, keys []*slhdsa.PrivateKey) {
...
func authAddress(keys []*slhdsa.PrivateKey, addr common.Address, chainID *uint256.Int, nonce uint64) []types.SetCodeAuthorization {
...
func ExecWithSK(backend *ethclient.Client, sk *slhdsa.PrivateKey, addr common.Address, nonce uint64, data []byte, blobs bool) *types.Transaction {
	cl, _ := helper.GetRealBackend()
